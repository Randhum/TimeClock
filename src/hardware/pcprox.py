...
Modified 2025 for TimeClock Project
...
class PcProx:
    ...
    def read(self) -> Optional[bytes]:
        ...
        try:
            ...
            # Handle possible USB errors or disconnection
    ...
    def set_halt_keyboard_send(self, halt: bool) -> None:
        """
        Optimized helper to set bHaltKBSnd and apply immediately (volatile).
        This stops the reader from acting as a keyboard.
        """
        config = self.get_config()
        if config.bHaltKBSnd != halt:
            config.bHaltKBSnd = halt
            # Only need to write Page 0
            config.set_config(self, pages=[0])
            self.end_config()
    ...
    def get_tag(self) -> Optional[Tuple[bytes, int]]:
        ...
        # Check for None (read error) or Empty bytes (no data)
        if not card_data or card_data == NULL_MSG:
            return None

        # This can be skipped without issue if there is no card there
        card_info = self.interact(b'\x8e')
        if not card_info or card_info == NULL_MSG:
            return None
