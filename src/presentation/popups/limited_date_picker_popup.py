"""
Limited date picker popup with min/max date constraints.
"""
import datetime
import logging
...
class LimitedDatePickerPopup(Popup):
    """Date picker with min/max date limits"""
    selected_date = ObjectProperty(None, allownone=True)
    
    def __init__(self, current_date=None, min_date=None, max_date=None, on_select=None, **kwargs):
        super().__init__(**kwargs)
        self.on_select_callback = on_select
        self.min_date = min_date
        self.max_date = max_date or datetime.date.today()
        ...
        # Ensure current_date is within range
        ...
        self.prev_month_btn = prev_month_btn
        self.next_month_btn = next_month_btn
        ...
        if self.max_date == datetime.date.today():
    ...
    def _change_month(self, delta):
        """Change displayed month, respecting limits"""
        ...
        new_display = datetime.date(year, month, 1)
        
        # Check if new month is within range
        if self.min_date:
            min_month_start = datetime.date(self.min_date.year, self.min_date.month, 1)
            if new_display < min_month_start:
                return  # Can't go before min_date
        
        max_month_start = datetime.date(self.max_date.year, self.max_date.month, 1)
        if new_display > max_month_start:
            return  # Can't go after max_date
        
        self.display_date = new_display
    ...
    def _update_selected_label(self):
        ...
        try:
            ...
            if self._is_date_valid(date):
                self.selected_date_label.text = date.strftime("%d.%m.%Y")
            else:
                self.selected_date_label.text = "Ungültig"
        except ValueError:
            self.selected_date_label.text = ""
    
    def _update_calendar(self):
        """Update the calendar grid with days, disabling invalid dates"""
        ...
        for day in range(1, days_in_month + 1):
            ...
            is_valid = self._is_date_valid(date)
            
            if is_selected and is_valid:
                bg_color = (0.2, 0.6, 0.9, 1)
                text_color = (1, 1, 1, 1)
            elif is_today and is_valid:
                ...
                bg_color = (0.2, 0.2, 0.2, 1)  # Dark gray for disabled
    ...
    def _select_day(self, day):
        ...
        if self._is_date_valid(date):
            ...
            for i, btn in enumerate(self.day_buttons):
                ...
                is_valid = self._is_date_valid(date_check)
    ...
    def _confirm_date(self):
        ...
        try:
            ...
            if not self._is_date_valid(selected_date):
                App.get_running_app().show_popup("Fehler", "Datum außerhalb des erlaubten Bereichs")
                return
